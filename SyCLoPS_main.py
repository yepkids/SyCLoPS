...
#If your dataset is not on a unstructured grid (i.e., not lon-lat grid), you should specify a connectivity file below and set use_connect to True. See the manual section 2.5 for more instructions.
use_connect = False  
connectivity_file = "connect_healpix_grid_zoom_8_example_filename.txt"

use_250hPa_only = False # Set to True if your dataset only contains 250 hPa level data for upper-level wind (U and V) and geopotential height (Z or H).
...
inputfile_DN = f"file_list/{model_data_name}_lpsnode_in.txt" #"This txt file should contain the list of existing input files that contains all the necessary variables for the whole procedure. \
...
outputfile_SN = f"out_track/{model_data_name}_lpstracks.csv" #This is the single output track file's filename in StitchNodes.
inputfile_VP = inputfile_DN #This txt file should contain the list of U and V at 850 hPa and 925 hPa levels, and it can be your inputfile for DetectNodes.
...
detect_nodes_cmd = [
    ...
    f"_VECMAG({uas_name},{vas_name}),max,3.0;" #WS ##This is an optional parameter (10m max wind speed) for reference purpose.
    ...
    f"{zs_name},min,0;" #ZS ## This parameter is not necessary if the dataset contains NaNs or missing values where 850 hPa level is below the surface. If you removed this, please also remove "ZS" in the StitchNodes command below
...
print("\nAll TempestExtremes commands executed successfully (double check to make sure no other errors are reported). Now running SyCLoPS_classifier.py to classify LPSs.\n")
